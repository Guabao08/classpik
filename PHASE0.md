...
**Also do:** Andy repeats Part A on DukeHub (PeopleSoft) — different enough that it needs its own answer.
...
Do B and A1/A2 now. A3 waits for your window. **Phase 1 does not depend on A3 at all** — start building it.
...
- [ ] What does a **failure** response look like — class full, time conflict, no time ticket, holds on account? We need to parse these to report useful errors.
...
- Note anything that looks like bot detection. If registration is CAPTCHA-gated, say so immediately — that changes the plan.
